...
exports.atualizarProduto = exports.deletarProduto = exports.cadastrarProduto = exports.listarProdutoPorId = exports.listarProdutos = void 0;
...
async function listarProdutos(req, res) {
    const listaProdutos = await Produto_1.produtos.find().populate('categoria').clone();
    if (!listaProdutos) {
...
async function listarProdutoPorId(req, res) {
    try {
        const { id } = req.params;
        const produto = await Produto_1.produtos.findById(id).clone();
        if (!produto) {
...
async function cadastrarProduto(req, res) {
    try {
        const produto = await Produto_1.produtos.create(req.body);
...
async function deletarProduto(req, res) {
    try {
        const { id } = req.params;
        const produto = await Produto_1.produtos.findByIdAndDelete(id).clone();
        if (!produto) {
...
async function atualizarProduto(req, res) {
    try {
        const { id } = req.params;
        const produto = await Produto_1.produtos.findByIdAndUpdate(id, { $set: req.body }).clone();
        if (!produto) {
